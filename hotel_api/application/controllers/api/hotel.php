...
class Hotel extends REST_Controller {
    ...
    public function index_get(){
        $id = $this->get('id');
        if($id != ''){
            $data_mhs = $this->hotel_model->gethotel($id);
        }else{
            $data_mhs = $this->hotel_model->gethotel();
        }
        if($data_mhs == true){
          $this->response(array(
            'status'  => 'true',
    ...
    public function tambah_post(){
        if($this->hotel_model->tambahDataHotel($_POST) > 0) {
          $this->response(array(
            'status'  => 'true',
            'code'    => '200',
    ...
    public function ubah_post(){
      if($this->hotel_model->ubahDataHotel($_POST) > 0) {
        $this->response(array(
          'status'  => 'true',
          'code'    => '200',
    ...
    public function hapus_get($id){
      if($this->hotel_model->hapusDataHotel($id)) {
         $this->response(array(
          'status'  => 'true',
          'code'    => '200',
